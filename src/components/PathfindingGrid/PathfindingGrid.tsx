...
const Grid = () => {
  ...
  const selectedAlgorithmIsUnweighted = useCallback(() => {
    return (
      pv.algorithm === PathfindingAlgorithm.bfs ||
      pv.algorithm === PathfindingAlgorithm.dfs
    );
  }, [pv.algorithm]);
  ...
  const getColorClass = (node: number[]) => {
    ...
    if (!selectedAlgorithmIsUnweighted() && pv.weights.includes(pos))
      return " weight";
  ...
  const getButtonClassName = (btnMode: DrawingMode) => {
    return (
      "grid-toolbox__button" + (drawingMode === btnMode ? " selected" : "")
    );
  };
  ...
  return (
    <div className="grid-container">
      ...
      <div className="grid-toolbox">
        ...
        <button onClick={clearDrawings} id="clear">
          Erase all
        </button>
      </div>
    </div>
